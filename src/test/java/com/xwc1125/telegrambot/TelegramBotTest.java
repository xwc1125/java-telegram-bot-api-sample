...
import com.xwc1125.telegrambot.passport.*;
import com.xwc1125.telegrambot.request.*;
import com.xwc1125.telegrambot.response.*;
...
//        }
    ...
    public void editMessageCaption() {
        ...
        SendResponse sendResponse = (SendResponse) bot.execute(new EditMessageCaption(chatId, messageId)
                .caption(text)
    ...
    public void editMessageReplyMarkup() {
        ...
        BaseResponse response = bot.execute(new EditMessageReplyMarkup(chatId, messageId).replyMarkup(keyboard));
        assertTrue(response.isOk());
...
//                .cacheTime(1));
    ...
    public void sendMessage() {
        ...
        // 强制回复
//        sendResponse = bot.execute(new SendMessage(chatId, "reply this message").replyMarkup(new ForceReply()));
...
//        MessageTest.checkTextMessage(sendResponse.message());
        ...
        InlineKeyboardMarkup inlineKeyboard = new InlineKeyboardMarkup(
                new InlineKeyboardButton[]{
                        ...
                        new InlineKeyboardButton("Share").url("http://t.me/xwc1125_bot?game=" + "lottery"),
                });
        sendResponse = bot.execute(new SendMessage(chatId, "inlineKeyboard")
...
//        MessageTest.checkForwardedMessage(response.message());
        ...
        Message message;
        message = bot.execute(new ForwardMessage(channelName, channelName, messageId)).message();
        assertNotNull(message.authorSignature());
        assertNotNull(message.forwardSignature());

        message = bot.execute(new ForwardMessage(chatId, groupId, messageId)).message();
...
//                        // 切换
        ...
        GameTest.check(game);
        assertEquals(gameShortName, game.description());
    }
    ...
    public void setGameScore() {
        ...
        SendResponse sendResponse = (SendResponse) bot.execute(
                new SetGameScore(chatId, res + 1, chatId, messageId).force(true).editMessage(false).disableEditMessage(true));
